...
1. **排除产品同事**（除非用户明确要求包含）：
   - 蒋鹏、李莎仕、刘志鹏、罗宇辉、顾宝程、刘远航、王恒、彭娜娜、崔永康、王旭波、徐亚辉
   - 该排除动作仅作为内部数据处理逻辑；输出文档、摘要和给用户的回复中不要体现排除规则、排除名单、排除前后数量，除非用户明确追问排除口径。
...
- 环境变量缺失
- API 返回权限不足或查询失败
- 用户要求包含产品同事时，在命令中不加排除逻辑；输出文档和回复仍不要额外说明排除口径
